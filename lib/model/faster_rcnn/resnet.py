...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=1000, honey=None):
...
def resnet101(honey=None, pretrained=False):
  ...
  if honey == None:
    honey = 33 * [10]
  model = ResNet(Bottleneck, [3, 4, 23, 3], honey = honey)
  if pretrained:
...
class resnet(_fasterRCNN):
  def __init__(self, classes, num_layers=101, pretrained=False, class_agnostic=False ,honey = None ,honey_ckpt = None):
    self.model_path = honey_ckpt
  ...
  def _init_modules(self):
    resnet = resnet101(self.honey)

    if self.pretrained == True:
      print("Loading pretrained weights from %s" %(self.model_path))
      bestckpt = torch.load(self.model_path)
      resnet.load_state_dict(bestckpt['state_dict'])
